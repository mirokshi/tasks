<template>
  <span>
      <span v-if="!editing" @dblclick="editing=true"> <!-- EDITA -->
          <slot></slot>
          <svg @click="editing=true" class="h-5 w-4.5 " xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M12.3 3.7l4 4L4 20H0v-4L12.3 3.7zm1.4-1.4L16 0l4 4-2.3 2.3-4-4z"/></svg>
      </span>

      <span v-if="editing" @keyup.esc="editing=false" @keyup.enter="edit">  <!--TERMMINA DE EDITAR -->
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M0 11l2-2 5 5L18 3l2 2L7 18z"/></svg>
          <input type="text" value="">

      </span>
  </span>
</template>
<script>
    export default {
        name: 'EditableText',
        data() {
            return {editing: false}
        },

        methods: {
            edit() {
             this.editing=false
            }
        }
    }
</script>